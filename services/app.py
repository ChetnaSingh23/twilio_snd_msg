...
def connect_db():
    ...
    try:
        db = MySQLdb.connect(host=config.get('localhost'),  # your host, usually localhost
                         ...
                         db=config.get("news"), cursorclass=MySQLdb.cursors.DictCursor,sql_mode="STRICT_TRANS_TABLES")  # name of the data base
        return db
    except:
...
def index():
    ...
    query="""INSERT into video(vid,vidname,video,vidsize)values(%s,%s,%s,%s)"""
